...
def derive_combined_mass(params, plot=False, n_samples=10000000, tables=False):
    ...
    if 'KIN' in params.keys():
        ...
        i[i>90] = i[i>90] - 180
        i[i<0] = -i[i<0]
    elif 'XDOT' in params.keys() and 'XDOT_ERR' in params.keys():
        ...
        try:
            ...
            try:
                pmelat = np.random.normal(loc=params["PMRA"], scale=params["PMRA_ERR"], size=n_samples)
                pmelong = np.random.normal(loc=params["PMDEC"], scale=params["PMDEC_ERR"], size=n_samples)
                pm_tot = np.sqrt(pmelat**2 + pmelong**2)
            except KeyError:
                pmelat = np.random.normal(loc=params["PMLAMBDA"], scale=params["PMLAMBDA_ERR"], size=n_samples)
                pmelong = np.random.normal(loc=params["PMBETA"], scale=params["PMBETA_ERR"], size=n_samples)
                pm_tot = np.sqrt(pmelat**2 + pmelong**2)
...
#        plt.close()
    ...
    #omdot_kop = 2.78e-7 * 1/np.sin(inc*np.pi/180) * (pmelong*np.cos(Omega) + pmelat*np.sin(Omega))
#    print(np.mean(omdot_kop), np.median(omdot_kop), np.std(omdot_kop))
