...
def get_data(tim, flag="-group"):
    ...
    for line in tim:
        ...
        ind = np.argwhere([x.strip() == flag for x in line])
...
def average_timfile(parfile, timfile, outfile=None, fd_correct=True,
    ...
    with open(outfile, "w+") as f:
        ...
        f.write("# Averaged .tim file prepared at {0}:{1}:{2} (UTC, 12h) on {3}/{4}/{5} \n".
                format(now.tm_hour,
                       ...
                       now.tm_year))
        f.write(
            "# Using Daniel Reardon's 'frequency_average.py' for PPTA-DR2 \n")
        f.write("# \n")
    with open(outfile, "a+") as f:
    ...
    if fit_dm:
        ...
        results = fitter(dm_model, dm_params,
                         (freqs, toas, errs, files, params, 1/errs),
                         mcmc=False)
...
if __name__ == '__main__':
    datadir = '/Users/dreardon/Desktop/Timing_analysis_of_the_PPTA_data_release_2/ppta_dr2e_ephemerides/'
