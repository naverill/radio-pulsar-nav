...
sys.path.insert(0, '/Users/dreardon/Git/ppta_dr2_ephemerides/parameterComparisonScripts/')
...
datadir = '/home/naverill/workspace/rpnav/rpnav/simulate/demo/ppta_data/'
outdir = '/home/naverill/workspace/rpnav/rpnav/simulate/demo/ppta_data/output/'
...
for par in parfiles:
    ...
    params = read_par(par)
    timfile = read_tim(datadir + "/" + psrname + ".tim")
    files = []
    for line in timfile:
        file = line.split()[0]
        files.append(file)
    ...
    nobs = len(uniq_files)
    # print("Number of unique observations:", nobs)
    #import sys
    #sys.exit()
    ...
    if 'ECC' in params.keys():
        # print("x * ecc**2 = ", params['A1']*params['ECC']**2)
        # print("wrms / sqrt(nobs)", params['TRES']*10**-6/np.sqrt(nobs))
        if params['A1']*params['ECC']**2 < \
           0.1*params['TRES']*10**-6/np.sqrt(nobs): # or 'J1600' in psrname or 'J1022' in psrname:
        ...
        # print("x * ecc**2 = ", params['A1']*ecc**2)
        # print("wrms / sqrt(nobs)", params['TRES']*10**-6/np.sqrt(nobs))
        if params['A1']*ecc**2 > \
           0.1*params['TRES']*10**-6/np.sqrt(nobs):
    ...
    if 'XDOT' in params.keys():
        ...
        Omega = np.random.normal(loc=25, scale=5, size=n_samples)*np.pi/180
        ...
        #plt.hist(i, density=True, bins=500)
        #plt.ylabel('Probability')
        #plt.xlabel('Inclination (deg)')
        #yl = plt.ylim()
        ##plt.plot([32, 32], yl, 'r--')
        ##plt.plot([180-32, 180-32], yl, 'r--')
        #plt.ylim(yl)
        #plt.xlim([0,90])
        #plt.grid()
        #plt.show()
...
#            f.write("V_R" + '\t' + str(round(v_r,1)) + '\t' + str(round(f2_err/f2 * v_r, 1)) + '\n')
    ...
    if 'EPS1DOT' in params.keys():
        ...
        omdot_posterior = omdot_posterior[cut]
        ecc_posterior = ecc_posterior[cut]
        plt.hist(omdot_posterior, bins=20)
        plt.xlabel('omdot')
        plt.show()
